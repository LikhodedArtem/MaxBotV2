...
async def help(message: Message) -> None:
    ...
    status = create_payload_status(type="list", action="view")
    await message.status(status)


@broker.check(Event.STATUS_CALLBACK({"type": "list", "action": "view"}))
async def view_list(message: Message) -> None:
    await message.answer("view_list")
